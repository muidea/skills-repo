...
### 2.3 Module 在服务中的注册与生命周期

在基于 `magicCommon/framework` 的典型项目中，实现一个模块要参与到服务运行，**只需要两处注册**：

1. **模块自身注册到框架 `module` 容器**
2. **应用入口 `main.go` 里通过匿名导入模块包**

#### 2.3.1 模块侧注册（`module.go`）

所有模块都通过 `init + module.Register` 完成自注册，示例（参考 `internal/modules/kernel/panel/module.go`）：
...
import (
	...
	// 可选：根据需要获取 Persistence / RouteRegistry 能力
	ipc "github.com/muidea/magicModulesRepo/initiators/persistence/pkg/common"
	irc "github.com/muidea/magicModulesRepo/initiators/routeregistry/pkg/common"
...
// Setup 在框架启动阶段被调用，用于通过 Initiator 获取依赖，并初始化 biz / service
func (s *{ModuleName}) Setup(eventHub event.Hub, backgroundRoutine task.BackgroundRoutine) (err *cd.Error) {
	var routeRegistryHelperVal irc.RouteRegistryHelper
	routeRegistryHelperVal, routeRegistryHelperErr := initiator.GetEntity(irc.RouteRegistryInitiator, routeRegistryHelperVal)
	if routeRegistryHelperErr != nil {
...
// Run 在框架 Run 阶段被调用，通常负责 Biz 初始化 + Service 注册路由
func (s *{ModuleName}) Run() (err *cd.Error) {
	err = s.bizPtr.Initialize()
	if err != nil {
		return
...
- 只要在 `init` 中调用 `module.Register(New())`，模块就会被框架识别并参与 `Setup/Run/Teardown` 生命周期。
- 在 `Setup` 中通过 `initiator.GetEntity` 获取持久化和路由注册能力，然后构造 `biz` 和 `service`。
- 在 `Run` 中先初始化 `biz`，再调用 `service.RegisterRoute()` 完成路由注册。

#### 2.3.2 应用侧注册（`application/{app_name}/cmd/main.go`）

应用入口通过**匿名导入（`_` 导入）**完成模块加载，下面是一个通用示例：
...
import (
	...
	// 注册框架 Initiator
	_ "github.com/muidea/magicModulesRepo/initiators/persistence"
	_ "github.com/muidea/magicModulesRepo/initiators/pprof"
	_ "github.com/muidea/magicModulesRepo/initiators/routeregistry"
	_ "github.com/muidea/magicModulesRepo/modules/blocks/cas"

	// 匿名导入本项目模块（**关键：触发各模块 init() -> module.Register**）
	_ "{module_path}/internal/modules/kernel/base"
...
func main() {
	...
	fmt.Printf("buildTime:%s, gitCommit:%s\n", buildTime, gitCommit)

	// 使用 HoldService：内部会调用 initiator.Setup / module.Setup
	err := application.Startup(service.HoldService())
	if err != nil {
		slog.Error("application.Startup err", "error", err.Error())
		return
	}

	err = application.Run()
	if err != nil {
		slog.Error("application.Run err", "error", err.Error())
		return
	}
...
1. 程序启动时，`main.go` 匿名导入模块包 → 各模块 `init()` 被执行 → 调用了 `module.Register(New())`。
2. `application.Startup(service.HoldService())` 内部：
   - 调用 `service.Startup()` → 先执行 `initiator.Setup(eventHub, backgroundRoutine)`，再执行 `module.Setup(...)`，完成所有模块的 `Setup`。
3. `application.Run()` 内部：
   - 执行 `service.Run()` → 先运行 `initiator.Run()`，再执行 `module.Run()`，逐个调用各模块的 `Run()`，此时会完成路由注册等操作。
4. 收到退出信号后，`application.Shutdown()` 调用 `service.Shutdown()`，最终执行 `module.Teardown()` 和 `initiator.Teardown()`。

通过以上机制，**新增模块只要完成 `module.go` 的注册 + 在应用 `main.go` 匿名导入即可自动接入整个服务生命周期**。
...
### 3.5 使用 magicEngine 进行路由注册

本项目的 HTTP 路由由 `github.com/muidea/magicEngine/http` 提供，典型用法有两种：

1. **直接使用 `engine.RouteRegistry`（不带权限控制）**
2. **通过 `toolkit.RoleRouteRegistry` 间接使用（带权限控制）**

#### 3.5.1 直接使用 `engine.RouteRegistry`

适用于网关类或内部服务，直接基于 HTTP 方法 + URI 注册处理函数，例如（可参考 `internal/modules/kernel/gateway/service/service.go`）：
...
import (
	...
	"{module_path}/internal/modules/kernel/gateway/biz"
)
...
func New(routeRegistry engine.RouteRegistry, bizPtr *biz.Gateway) *Gateway {
	return &Gateway{
...
func (s *Gateway) RegisterRoute() {
	// 示例：注册一个简单的 GET 路由
	s.routeRegistry.AddHandler("/api/gateway/ping", engine.GET, s.ping)
}

func (s *Gateway) ping(ctx context.Context, res http.ResponseWriter, req *http.Request) {
...
- `engine.RouteRegistry` 提供 `AddHandler(uriPattern, method, handler)` 能力，`method` 使用 `engine.GET/POST/PUT/DELETE` 等常量。
- URI 支持参数与通配符，内部通过 `PatternFilter` 做匹配（见 `vendor/github.com/muidea/magicEngine/http/route.go`）。
- 一般在模块的 `Run()` 阶段调用 `service.RegisterRoute()` 完成所有路由注册。

如需设置 API 版本，可以在绑定前调用：

```go
routeRegistry.SetApiVersion("/api/v1")
routeRegistry.AddHandler("/gateway/ping", engine.GET, s.ping) // 实际匹配路径为 /api/v1/gateway/ping
```

#### 3.5.2 通过 `toolkit.RoleRouteRegistry` 使用 magicEngine（带权限）

大多数业务模块（如 `panel` / `base`）通过 `magicBase` 提供的 `RoleRouteRegistry` 进行路由注册，它内部同样基于 `magicEngine.RouteRegistry`，但封装了权限控制逻辑。

典型模式（参考 `internal/modules/kernel/panel/service/service.go`）：
...
func (s *{ModuleName}) RegisterRoute() {
	// 带权限控制的路由
	s.roleRouteRegistry.AddPrivilegeHandler(common.{RouteName}, engine.GET, bc.ReadPermission, s.handler)
	s.roleRouteRegistry.AddPrivilegeHandler(common.{RouteName}, engine.POST, bc.WritePermission, s.create)

	// 无需登录 / 权限的公开路由
	s.roleRouteRegistry.AddHandler(common.{PublicRouteName}, engine.GET, s.publicHandler)
}
```

**获取 `RoleRouteRegistry` 的方式**（在模块 `Setup` 中）：
...
在 `base` 模块中还演示了设置 API 版本的做法（`internal/modules/kernel/base/service/service.go`）：

```go
func (s *Base) BindRegistry(roleRouteRegistry toolkit.RoleRouteRegistry) {
	s.roleRouteRegistry = roleRouteRegistry
	s.roleRouteRegistry.SetApiVersion(bc.ApiVersion) // 统一设置 API 版本前缀
}
```

**总结：**

- **简单内部路由**：可以直接使用 `engine.RouteRegistry.AddHandler` 注册。
- **需要权限控制的业务路由**：推荐通过 `RoleRouteRegistry.AddPrivilegeHandler` 注册，并使用 `bc.ReadPermission/bc.WritePermission/bc.DeletePermission` 管理访问权限。
- 无论哪种方式，最终都是基于 `magicEngine/http` 的路由匹配与分发机制。
