...
- 对外 HTTP API 常量应定义为最终 REST 路径，而不是历史伪 CRUD 路径；前端、文档、测试都使用同一个最终路径。
- `AddHandler`、`AddCasHandler`、`AddPrivilegeHandler` 都会先调用 `NormalizeURI`，因此伪 CRUD 路径会被改写：
...
- 新增自定义 API 时，避免把路径命名成 `*/filter/`、`*/query/:id`、`*/insert/`、`*/update/:id`、`*/delete/:id`，除非明确就是要复用标准 CRUD 标准化行为。
- 如果业务期望路径是 `/portal/access-logs/`，常量就直接定义为 `/portal/access-logs/`，不要定义成 `/portal/access-log/filter/` 再依赖隐式改写。
...
- 标准 CRUD：注册常量使用 `filter/query/insert/update/delete`，对外文档和前端使用归一化后的 REST 路径。
- 自定义 API：注册常量直接使用最终 REST 路径，避免误触发 `NormalizeURI` 的操作词。
- 写出 route contract：原始常量、归一化路径、`bc.ApiVersion` 后路径、HTTP method、CAS/Role/Plain 类型、权限值、前端调用路径。
