...
- 决定事件使用 `Post` 还是 `Send`
- 设计跨 module 的事件通知、同步结果、lane 顺序和后台任务协同
- 排查应用 shutdown 后任务、事件、timer、goroutine 残留
- 处理 `BG_TASK_QUEUE_SIZE`、`HUB_EVENT_QUEUE_SIZE` 等运行时容量配置

不使用本 skill：

- 只新增 `initiator` / `module` 生命周期，优先使用 `go-module-initiator-lifecycle`
...
- `service.Startup` 再传给 `initiator.Setup` 和 `module.Setup`。
...
- `Run` 成功后才能认为服务 ready；不要在 module 内私自标记全局 ready。
- 退出路径必须调用 `application.Shutdown()`，不要只关闭 HTTP server 或某个 module。
...
- 取消订阅应放在 module 或 runtime 的关闭路径中。

## BackgroundRoutine 使用规则

- module 或 initiator 不要裸起长期 goroutine，优先使用 `BackgroundRoutine`。
...
- `initiator.Setup` 和 `module.Setup` 只能保存和接线 hub/background，不应启动长期阻塞逻辑。
- `initiator.Run` / `module.Run` 才注册 timer、订阅事件或启动业务运行逻辑。
- `Teardown` 或 `Shutdown` 要释放订阅、listener、timer context 和自持资源。
- 事件发布不能早于订阅方依赖准备完成，除非业务明确允许丢弃或延迟处理。
- 跨 module 通信优先用事件或显式 helper 接口，不要通过全局变量偷取 hub。
...
```bash
GOCACHE=/tmp/go-application-event-runtime-gocache go test ./internal/initiators/... ./internal/modules/... -count=1
```
